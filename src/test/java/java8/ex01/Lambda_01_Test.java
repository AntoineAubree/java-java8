...
public class Lambda_01_Test {
	...
	public void test_filter_by_firstname() throws Exception {
		...
		PersonPredicate personPredicate = person -> person.getFirstname().equals("first_10");
		List<Person> result = filter(personList, personPredicate);
	...
	public void test_filter_by_password() throws Exception {
		...
		PersonPredicate personPredicate = person -> person.getAge() > 49 && DigestUtils.sha512Hex(person.getPassword()).equals(passwordSha512Hex);
		List<Person> result = filter(personList, personPredicate);
